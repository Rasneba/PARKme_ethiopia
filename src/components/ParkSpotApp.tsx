...
function CityMap({ onBook }: { onBook: (place?: typeof places[number]) => void }) {
  ...
  return (
    <section className="map-column" aria-label="Map of available parking spots in Addis Ababa">
      ...
      <div className="map-attribution">© OpenStreetMap · ParkAddis map</div>
    </section>
  );
}
    ...
    <div className="overlay" role="dialog" aria-modal="true" aria-label="Reserve your parking space">
      <div className={`booking-modal ${complete ? "ticket-complete" : ""}`}>
        ...
        {!complete ? <>
          ...
          <section className="booking-section"><div className="section-title"><span><Icon name="wallet" size={18} /> Payment method</span><button className="tiny-action">+ Add</button></div><div className="payment-options"><button className={payment === "wallet" ? "selected" : ""} onClick={() => setPayment("wallet")}><span className="payment-symbol wallet-symbol"><Icon name="wallet" size={17} /></span><span>ParkAddisWallet <small>Balance: 250 ETB</small></span><i className="radio" /></button><button className={payment === "telebirr" ? "selected" : ""} onClick={() => setPayment("telebirr")}><span className="payment-symbol telebirr-symbol">t</span><span>telebirr <small>Instant payment</small></span><i className="radio" /></button></div></section>
          <section className="coupon-row"><Icon name="sparkle" size={17} /><input value={coupon} onChange={(event) => { setCoupon(event.target.value); setCouponApplied(false); }} placeholder="Promo or coupon code" /><button onClick={() => coupon.trim() && setCouponApplied(true)}>{couponApplied ? "Applied" : "Apply"}</button></section>
          ...
          <button className="confirm-booking" onClick={() => setComplete(true)}>Confirm & pay {total} ETB <Icon name="arrow" size={18} /></button><p className="secure-note"><Icon name="shield" size={15} /> Secured by ParkAddis payments</p>
        </> : <>
          ...
          <div className="digital-ticket"><FlagRibbon /><span className="ticket-notch ticket-left" /><span className="ticket-notch ticket-right" /><div className="ticket-topline"><span>PARKADDIS PARKING PASS</span><StatusDot text="Valid today" /></div><h3>{place.name}</h3><p>{place.address}</p><div className="ticket-dates"><div><small>ARRIVAL</small><b>10:30 AM</b><span>Thu, 27 June</span></div><div><small>DURATION</small><b>{duration} hours</b><span>Until {10 + duration}:30 PM</span></div><div><small>SPACE</small><b>B · 27</b><span>Level B</span></div></div><div className="ticket-line" /><div className="qr-area"><button className={`qr-code ${scanned ? "scanned" : ""}`} onClick={() => setScanned(true)} aria-label="Scan check-in QR code"><span className="qr-corner top-left" /><span className="qr-corner top-right" /><span className="qr-corner bottom-left" /><span className="qr-corner bottom-right" /><i /><i /><i /><i /><i /><i /></button><div><p>{scanned ? "Gate checked in" : "CHECK IN AT GATE"}</p><b>{scanned ? "Welcome to Unity Park" : "Scan your ticket QR code"}</b><button className="scan-button" onClick={() => setScanned(true)}><Icon name="scan" size={16} /> {scanned ? "Check-in complete" : "Open scanner"}</button></div></div></div>
