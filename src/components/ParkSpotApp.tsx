    ...
    <div className="overlay" role="dialog" aria-modal="true" aria-label="Reserve your parking space">
      <div className={`booking-modal ${complete ? "ticket-complete" : ""}`}>
        ...
        {!complete ? <>
          ...
          <button className="confirm-booking" onClick={() => setComplete(true)}>Confirm & pay {total} ETB <Icon name="arrow" size={18} /></button><p className="secure-note"><Icon name="shield" size={15} /> Secured by Prakme payments</p>
        </> : <>
          ...
          <div className="digital-ticket"><FlagRibbon /><span className="ticket-notch ticket-left" /><span className="ticket-notch ticket-right" /><div className="ticket-topline"><span>PRAKME PARKING PASS</span><StatusDot text="Valid today" /></div><h3>{place.name}</h3><p>{place.address}</p><div className="ticket-dates"><div><small>ARRIVAL</small><b>10:30 AM</b><span>Thu, 27 June</span></div><div><small>DURATION</small><b>{duration} hours</b><span>Until {10 + duration}:30 PM</span></div><div><small>SPACE</small><b>B · 27</b><span>Level B</span></div></div><div className="ticket-line" /><div className="qr-area"><button className={`qr-code ${scanned ? "scanned" : ""}`} onClick={() => setScanned(true)} aria-label="Scan check-in QR code"><span className="qr-corner top-left" /><span className="qr-corner top-right" /><span className="qr-corner bottom-left" /><span className="qr-corner bottom-right" /><i /><i /><i /><i /><i /><i /></button><div><p>{scanned ? "Gate checked in" : "CHECK IN AT GATE"}</p><b>{scanned ? "Welcome to Unity Park" : "Scan your ticket QR code"}</b><button className="scan-button" onClick={() => setScanned(true)}><Icon name="scan" size={16} /> {scanned ? "Check-in complete" : "Open scanner"}</button></div></div></div>
...
export default function PrakmeSpotApp() {
